...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的车位订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li v-for="item in cheweiOrderYesNoTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.cheweiName}}</td>
                            <td style="width: 60px;">{{item.cheweiAddress}}</td>
                            <td style="width: 100px;color: red">{{item.cheweiNewMoney}}</td>
                            <td style="width: 80px;">{{item.cheweiValue}}</td>
                            <td style="width: 80px;">{{item.cheweiOrderYesNoValue}}</td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 cheweiOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            cheweiOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(cheweiOrderYesNoTypes) {
                this.cheweiOrderYesNoTypes = cheweiOrderYesNoTypes;
                // 获取列表数据
                layui.http.request('cheweiOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    cheweiOrderYesNoTypes: vue.cheweiOrderYesNoTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('cheweiOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    cheweiOrderYesNoTypes: this.cheweiOrderYesNoTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`cheweiOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`cheweiOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('cheweiOrder/list', 'get',{
            page: 1,
            limit: limit,
            cheweiOrderYesNoTypes: vue.cheweiOrderYesNoTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('cheweiOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            cheweiOrderType: vue.cheweiOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
       ...
       function cheweiOrderYesNoTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chewei_order_yes_no_types", 'get', {}, function (res) {
               if(res.code == 0){

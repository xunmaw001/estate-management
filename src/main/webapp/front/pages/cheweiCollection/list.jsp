            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">车位收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="cheweiName" id="cheweiName" placeholder="车位名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="cheweiAddress" id="cheweiAddress" placeholder="车位地址" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <%--<div class="layui-inline" style="margin-bottom: 10px;">--%>
                              ...
                              <%--<div class="layui-input-inline">--%>
                                  <%--<select name="cheweiTypes" id="cheweiTypes" lay-filter="cheweiTypes">--%>
                                      <%--<option value="">--请选择--</option>--%>
                                      <%--<option v-for="item in cheweiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>--%>
                                  <%--</select>--%>
                              <%--</div>--%>
                          <%--</div>--%>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../chewei/detail.jsp?id='+item.cheweiId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.cheweiPhoto?item.cheweiPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../chewei/detail.jsp?id='+item.cheweiId)"><span>{{item.cheweiName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.cheweiNewMoney"><span style="color: red;">{{item.cheweiNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteCheweiCollection(item.id)">删除 {{item.cheweiName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 cheweiCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            cheweiCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteCheweiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('cheweiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#cheweiName').val() != null && jquery('#cheweiName').val() != "" && jquery('#cheweiName').val() != "null" ) {
                param['cheweiName'] = jquery('#cheweiName').val();
            }
                     
            if (jquery('#cheweiAddress').val() != null && jquery('#cheweiAddress').val() != "" && jquery('#cheweiAddress').val() != "null" ) {
                param['cheweiAddress'] = jquery('#cheweiAddress').val();
            }
                                                                                 
            var cheweiTypes = $("#cheweiTypes").find("option:selected").val();
            if (cheweiTypes != null && cheweiTypes != "" && cheweiTypes != "null") {
                            param['cheweiTypes'] = cheweiTypes;
                        }
                    
            // 当前表
                                                    // 获取列表数据
            http.request('cheweiCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function cheweiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chewei_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.cheweiTypesList = res.data.list;
               }
           });
       }

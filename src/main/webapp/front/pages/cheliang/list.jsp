            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">车辆展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="cheliangName" id="cheliangName" placeholder="车辆名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="cheliangTypes" id="cheliangTypes" lay-filter="cheliangTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in cheliangTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 cheliangContent 字段去除无用字符 如果字段长度大于60  就截取到60
            cheliangContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#cheliangName').val() != null && jquery('#cheliangName').val() != "" && jquery('#cheliangName').val() != "null" ) {
                param['cheliangName'] = jquery('#cheliangName').val();
            }
         
            var cheliangTypes = $("#cheliangTypes").find("option:selected").val();
            if (cheliangTypes != null && cheliangTypes != "" && cheliangTypes != "null") {
                param['cheliangTypes'] = cheliangTypes;
            }
         
            if (jquery('#chepai').val() != null && jquery('#chepai').val() != "" && jquery('#chepai').val() != "null" ) {
                param['chepai'] = jquery('#chepai').val();
            }
                                    // 获取列表数据
            http.request('cheliang/list', 'get', param, function(res) {
    ...
    // 当前表的字典表
       ...
       function cheliangTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=cheliang_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.cheliangTypesList = res.data.list;
               }
           });
       }

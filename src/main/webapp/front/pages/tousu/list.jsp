            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">投诉展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">投诉名称</label>
                          <div class="layui-input-inline">
                              <input type="text" name="tousuName" id="tousuName" placeholder="投诉名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="tousuTypes" id="tousuTypes" lay-filter="tousuTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in tousuTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="tousuYesNoTypes" id="tousuYesNoTypes" lay-filter="tousuYesNoTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in tousuYesNoTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 tousuContent 字段去除无用字符 如果字段长度大于60  就截取到60
            tousuContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#tousuName').val() != null && jquery('#tousuName').val() != "" && jquery('#tousuName').val() != "null" ) {
                param['tousuName'] = jquery('#tousuName').val();
            }
         
            var tousuTypes = $("#tousuTypes").find("option:selected").val();
            if (tousuTypes != null && tousuTypes != "" && tousuTypes != "null") {
                param['tousuTypes'] = tousuTypes;
            }
                         
            var tousuYesNoTypes = $("#tousuYesNoTypes").find("option:selected").val();
            if (tousuYesNoTypes != null && tousuYesNoTypes != "" && tousuYesNoTypes != "null") {
                param['tousuYesNoTypes'] = tousuYesNoTypes;
            }
                    // 获取列表数据
            http.request('tousu/list', 'get', param, function(res) {
    ...
    // 当前表的字典表
       ...
       function tousuTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=tousu_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.tousuTypesList = res.data.list;
               }
           });
       }
       ...
       function tousuYesNoTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=tousu_yes_no_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.tousuYesNoTypesList = res.data.list;
               }
           });
       }

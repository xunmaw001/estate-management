                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>缴费名称</label>
                                        <input id="jiaofeiName" name="jiaofeiName" class="form-control"
                                               placeholder="缴费名称">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>月份</label>
                                        <input id="jiaofeiTime-input" name="jiaofeiTime" type="text" class="form-control layui-input">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>缴费类型</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>缴费金额</label>
                                        <input id="jiaofeiYuMoney" name="jiaofeiYuMoney" class="form-control"
                                               onchange="jiaofeiYuMoneyChickValue(this)" placeholder="缴费金额">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>实收金额</label>
                                        <input id="jiaofeiShiMoney" name="jiaofeiShiMoney" class="form-control"
                                               onchange="jiaofeiShiMoneyChickValue(this)" placeholder="实收金额">
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>缴费详情</label>
                                        <input id="jiaofeiContentupload" name="file" type="file">
                                        <script id="jiaofeiContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var jiaofeiContentUe = UE.getEditor('jiaofeiContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="jiaofeiContent" id="jiaofeiContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#jiaofeiContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('jiaofeiContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.jiaofeiContent != null && ruleForm.jiaofeiContent != 'null' && ruleForm.jiaofeiContent != '' && $("#jiaofeiContentupload").length>0) {

            var jiaofeiContentUeditor = UE.getEditor('jiaofeiContentEditor');
            jiaofeiContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.jiaofeiContent != null){
                    mes = ''+ ruleForm.jiaofeiContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                jiaofeiContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#jiaofeiContentupload").length>0) {
            var jiaofeiContentEditor = UE.getEditor('jiaofeiContentEditor');
            if (jiaofeiContentEditor.hasContents()) {
                $('#jiaofeiContent-input').attr('value', jiaofeiContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function jiaofeiYuMoneyChickValue(e){
        ...
        function jiaofeiShiMoneyChickValue(e){
            var this_val = e.value || 0;
            var reg=/^[0-9]{0,6}(\.[0-9]{1,2})?$/;
            if(!reg.test(this_val)){
                e.value = "";
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yonghuId: "required",
                jiaofeiName: "required",
                jiaofeiTime: "required",
                jiaofeiTypes: "required",
                jiaofeiYuMoney: "required",
                // jiaofeiShiMoney: "required",
                jiaofeiContent: "required",
            },
            messages: {
                yonghuId: "用户不能为空",
                jiaofeiName: "缴费名称不能为空",
                jiaofeiTime: "月份不能为空",
                jiaofeiTypes: "缴费类型不能为空",
                jiaofeiYuMoney: "缴费金额不能为空",
                // jiaofeiShiMoney: "实收金额不能为空",
                jiaofeiContent: "缴费详情不能为空",
            }
        }).form();
    }
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#jiaofeiName").val(ruleForm.jiaofeiName);
        $("#jiaofeiTime-input").val(ruleForm.jiaofeiTime);
        $("#jiaofeiYuMoney").val(ruleForm.jiaofeiYuMoney);
        $("#jiaofeiShiMoney").val(ruleForm.jiaofeiShiMoney);
        $("#jiaofeiContent").val(ruleForm.jiaofeiContent);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $(".aaaaaa").remove();
            if(window.sessionStorage.getItem('addjiaofei') != null && window.sessionStorage.getItem('addjiaofei') !=""){//新增查询,修改不查
                // 查询当前登录账户的信息
                var userId = window.sessionStorage.getItem('userId');
                    yonghuSelectOne(userId);
            }
            // 缴费金额设置不可选
            $('#jiaofeiYuMoney').attr('readonly', 'readonly');
            // 实收金额设置不可选
            $('#jiaofeiShiMoney').attr('readonly', 'readonly');
        }else{
            // alert("未知情况.......");
        }
    }
